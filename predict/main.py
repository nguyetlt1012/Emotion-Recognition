...
face_haar_cascade = cv2.CascadeClassifier('predict/haarcascade_frontalface_default.xml')
...
while cap.isOpened():
    ...
    try:
        for (x,y, w, h) in faces:
            ...
            # image = cv2.cvtColor(roi_gray, cv2.COLOR_BGR2GRAY)
            # image_pixels = img_to_array(roi_gray)
            # image_pixels = np.expand_dims(image_pixels, axis = 0)
            # image_pixels /= 255
            predictions = model.predict(np.array([roi_gray]))
